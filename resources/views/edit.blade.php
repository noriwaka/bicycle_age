    ...
    <form action="{{ route('parts.update.all') }}" method="POST">
        ...
        @forelse ($parts as $index => $part)
            <div class="flex gap-4 items-center mb-4">
                ...
                <div>
                    <label for="part_distance_{{ $index }}" class="block text-sm font-medium text-gray-700">走行距離(km)</label>
                    <input type="number" name="parts[{{ $index }}][distance]" id="part_distance_{{ $index }}" placeholder="{{ $part->mileage }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                </div>
            </div>
        @empty
            <p>編集するパーツがありません。</p>
